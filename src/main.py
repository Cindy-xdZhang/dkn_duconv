...
def arg_config():
    ...
    net_arg.add_argument("--model_type", type=str, default='trans',
                         choices=['trans', 'gru'])
    net_arg.add_argument("--hidden_size", type=int, default=32)
...
def build_models(voc,config,checkpoint):
    ...
    if config.model_type =="gru":
        ...
        pos_embedding_layer=network.PositionalEncoding(WORD_EMBEDDING_DIM,n_position=200)
...
def train(config):
    ...
    print('-Building models   -model_type: '+str(config.model_type)+" ...")
...
def trainIter_gru(train_handler):
    ...
    for batch_idx, data in enumerate(train_loader):
        history,knowledge,responses=data["history"],data["knowledge"],data["response"]
        history,len_history=padding_transform(history)
        knowledge,len_knowledge=padding_transform(knowledge)
        responses,len_responses=padding_transform(responses)
        if config.use_gpu and USE_CUDA: 
