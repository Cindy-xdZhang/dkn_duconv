...
def calc_cover_rate(pair_list, ngram):
    ...
    if result[1]==0.0:result[1]+=1e-8#add by xdz
...
def calc_f1(data):
    ...
    for response, golden_response in data:
        golden_response = "".join(golden_response).encode("utf8")
        response = "".join(response).encode("utf8")
...
def convert_result_for_eval(sample_file, output_file_dir, output_file):
    """
    convert_result_for_eval
    """
    result_file=os.path.join(output_file_dir,"result.txt")
