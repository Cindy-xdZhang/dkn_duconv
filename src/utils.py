...
class Vocabulary:
    ...
    def addWord(self, word):
        if word not in self.word2index:
            ...
            self.word2count[word] += 1
    def idx_padding_corpus(self,train_data):
        """ 数字化 and padding every sentense in corpus 
            输入  "2, 23 ,45 ,56"格式字符串
            输出  [ 2, 23 ,45 ,56,1,2,2,2,2]int 列表->直接进网络
        """
        for item in train_data:
            item["history"]=(self.encoding_sentence(item["history"]))
            item["response"]=(self.encoding_sentence(item["response"]))
            item["knowledge"]=(self.encoding_sentence(item["knowledge"]))
    ...
    def encoding_sentence(self,sentence):
        ...
        if isinstance(sentence, str): 
            ...
            word_encoding = [str(PAD_token)] * MAX_TITLE_LENGTH
            point = 0
            for s in array:
            ...
            tokens_list = [self.encoding_sentence(t) for t in sentence]
            return tokens_list
...
##record 2020.0211: 把tokenize数字集合到word_index中
...
def entity_index(train_data):
    ...
    return voc
# tokenize数字在sample、泛化后，在建立词汇表以前   2020.0202
def tokenize(tokens):
    """
    tokenize
